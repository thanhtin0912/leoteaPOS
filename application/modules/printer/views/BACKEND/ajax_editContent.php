...
function showRequest(formData, jqForm, options) {
	...
	if(form.ipAdmincp.value == '' || form.cateAdmincp.value == ''){
		$('#txt_error').html('Please enter data information.');
		show_perm_denied();
		return false;
	}
}
...
function showResponse(responseText, statusText, xhr, $form) {
	...
	if(responseText[0]=='error-title-exists'){
		$('#txt_error').html('Đã tồn tại cài đặt này trong hệ thống');
		show_perm_denied();
		return false;
	}
			...
			<div class="portlet-body form">
				...
				<form id="frmManagement" action="<?=PATH_URL_ADMIN.$module.'/save/'?>" method="post" enctype="multipart/form-data" class="form-horizontal form-row-seperated">
					...
					<div class="form-body">
						...
						<div class="form-group">
							<label class="control-label col-md-2">Kiểu in:</label>
							<div class="col-md-10">
								<label class="radio-inline"><input type="radio" name="typeAdmincp" value="TEM" <?= isset($result->type) ? $result->type === 'TEM' ? 'checked' : '' : '' ?> > TEM</label>
								<label class="radio-inline"><input type="radio" name="typeAdmincp" value="BILL" <?= isset($result->type) ? $result->type === 'BILL' ? 'checked' : '' : 'checked' ?> > BILL</label>
							</div>
						</div>
						...
						<div class="form-group">
							...
							<div class="col-md-6">
								<select class="bs-select form-control" data-live-search="true" data-size="8" name="cateAdmincp" id="cateAdmincp">
									...
									<?php foreach ($stores as $key => $cate): ?>
										<?php  
											$select = '';
						...
						<div class="form-group">
							<label class="control-label col-md-2">Địa chỉ IP: <span class="required" aria-required="true">*</span></label>
						...
						<div class="form-group last">
							<label class="control-label col-md-2">Ghi chú: <span class="required" aria-required="true">*</span></label>
							<div class="col-md-10"><textarea name="noteAdmincp" id="noteAdmincp" cols="" rows="5" class="form-control"><?php if(isset($result->note)) { print $result->note; }else{ print '';} ?></textarea></div>
						</div>
