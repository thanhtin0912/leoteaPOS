...
class Printer_model extends CI_Model {
	...
	private $table = 'printer';
	private $table_store = 'stores';

	function getsearchContent($limit,$page){
		$this->db->select('n.*, c.name as store_name');
		$this->db->limit($limit,$page);
		$this->db->order_by('n.delete','ASC');
		$this->db->order_by($this->input->post('func_order_by'),$this->input->post('order_by'));
		if($this->input->post('title')!=''){
			$this->db->where('(c.`name` LIKE "%'.$this->input->post('title').'%")');
		}
		...
		$this->db->join(PREFIX.$this->table_store." c", 'c.id = n.storeId', "left");
		$query = $this->db->get();
	...
	function saveManagement($fileName=''){
		if($this->input->post('hiddenIdAdmincp')==0){
			$checkData = $this->checkData($this->input->post('cateAdmincp'), $this->input->post('typeAdmincp'),$this->input->post('hiddenIdAdmincp'));
			if($checkData){
				print 'error-title-exists.'.$this->security->get_csrf_hash();
				exit;
			}
			$data = array(
				'storeId'=> trim($this->input->post('cateAdmincp', true)),
				'ip'=> trim($this->input->post('ipAdmincp', true)),
				'note'=> trim($this->input->post('noteAdmincp')),
	...
	function checkData($storeId,$type,$id=0){
		$this->db->select('id');
	...
	function getCateDel(){
		...
		$query = $this->db->get(PREFIX.$this->table_store);
