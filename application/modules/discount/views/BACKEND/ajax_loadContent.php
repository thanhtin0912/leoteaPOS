...
<div class="dataTables_wrapper no-footer">
	...
	<div class="table-scrollable">
		<table class="table table-striped table-bordered table-hover dataTable no-footer">
			<thead>
				<tr role="row">
					<th class="table-checkbox sorting_disabled" width="25"><input type="checkbox" id="selectAllItems" onclick="selectAllItems(<?= (is_array($result) ? count($result) : 0) ?>)"></th>
					<th class="center sorting_disabled" width="35">No.</th>
					<th class="sorting" onclick="sort('name')" id="name">Tên</th>
					<th class="sorting" onclick="sort('code')" id="">Mã KM</th>
					<th class="sorting">Loại KM</th>
			...
			<tbody>
				...
				<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
					...
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?= $v->code; ?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?= ($v->type == 1) ? 'Tiền mặt' : (($v->type == 2) ? 'Phần trăm HD' : 'Số ly'); ?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?= $v->discount; ?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>">
						<?php if($v->type == 1 ) { echo "Đơn hàng > <strong>".number_format($v->condition)."</strong> giảm giá  <strong>".number_format($v->discount)."</strong>";} ?>
						<?php if($v->type == 3 ) { echo "Số ly lớn >= <strong>".number_format($v->condition)."</strong> tặng <strong>".number_format($v->discount)." ly giá thấp nhất</strong>";} ?>
						</a>
					</td>
				...
				<?php $i++;}
					...
					<tr style="background:#c6c6c6;" class="item_row<?=$i?> gradeX" role="row">
						...
						<td><?= $v->name; ?></td>
						<td><?= $v->code; ?></td>
						<td><?= ($v->type == 1) ? 'Tiền mặt' : (($v->type == 2) ? 'Phần trăm HD' : 'Số ly'); ?></td>
						<td><?= $v->discount; ?></td>
						<td><?= $v->order; ?></td>
						<td>
							<?php if($v->type == 1 ) { echo "Đơn hàng > <strong>".number_format($v->condition)."</strong> giảm giá  <strong>".number_format($v->discount)."</strong>";} ?>
							<?php if($v->type == 3 ) { echo "Số ly lớn >= <strong>".number_format($v->condition)."</strong> tặng <strong>".number_format($v->discount)." thấp nhất</strong>";} ?>
						</td>
