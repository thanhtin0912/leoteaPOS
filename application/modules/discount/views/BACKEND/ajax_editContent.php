			...
			<div class="portlet-body form">
				...
				<form id="frmManagement" action="<?=PATH_URL_ADMIN.$module.'/save/'?>" method="post" enctype="multipart/form-data" class="form-horizontal form-row-seperated">
					...
					<div class="form-body">
						...
						<div class="form-group">
							<label class="control-label col-md-2">Tên chương trình: <span class="required" aria-required="true">*</span></label>
						...
						<div class="form-group">
							<label class="control-label col-md-2">mã KM: <span class="required" aria-required="true">*</span></label>
							<div class="col-md-10"><input value="<?php if(isset($result->code)) { print $result->code; }else{ print '';} ?>" type="text" name="slugAdmincp" id="slugAdmincp" class="form-control"/></div>
						</div>
						<div class="form-group">
							<label class="control-label col-md-2">Kiểu KM:</label>
							<div class="col-md-10">
								<label class="radio-inline"><input type="radio" name="typeAdmincp" value="1" <?= isset($result->type) ? ($result->type == 1 ? 'checked' : '') : 'checked' ?> > Tiền mặt</label>
								<label class="radio-inline"><input type="radio" name="typeAdmincp" value="2" <?= isset($result->type) ? ($result->type == 2 ? 'checked' : '') : '' ?> > Phần trăm HD</label>
								<label class="radio-inline"><input type="radio" name="typeAdmincp" value="3" <?= isset($result->type) ? ($result->type == 3 ? 'checked' : '') : '' ?> > Số ly</label>
							</div>
						</div>
						<div class="form-group">
							<label class="control-label col-md-2">Điều kiện(<) <span class="required" aria-required="true">*</span></label>
							...
							<label class="control-label col-md-2">Giảm giá <span class="required" aria-required="true">*</span></label>
							<div class="col-md-3"><input value="<?php if(isset($result->discount)) { print $result->discount; }else{ print '';} ?>" type="text" name="discountAdmincp" id="discountAdmincp" class="form-control"/></div>					
						</div>
						<div class="form-group">
							<label class="control-label col-md-2">Mô tả:</label>
							<div class="col-md-8">
								<textarea rows="4" name="contentAdmincp" class="form-control"><?php if(isset($result->description)) { print $result->description; }else{ print '';} ?></textarea>
							</div>
						</div>
