...
class Toppings extends MX_Controller {
	...
	public function admincp_update($id=0){
		$this->load->model('categories_topping/categories_topping_model');
		...
		if($id!=0){
			...
			foreach ($products as $key => $pro) {
				...
				if(!is_null($toppings) || !empty($toppings) ) {
					if (($key = array_search($id, $toppings)) !== false) {
						array_push($productsTopping, $pro->id);
					}
				}
			}
		}
		$data = array(
			'cates'=>$this->categories_topping_model->getData(),
			'products'=> $this->products_model->getData(),
			'productsSelect' => $productsTopping,
			'cate_products'=>$this->categories_model->getData(),
			'result'=>$result[0],
			'module'=>$this->module,
			'id'=>$id
		);
