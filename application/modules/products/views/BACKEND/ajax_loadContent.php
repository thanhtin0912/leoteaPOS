		...
		<table class="table table-striped table-bordered table-hover dataTable no-footer">
			<thead>
				<tr role="row">
					...
					<th class="sorting" onclick="sort('image')" id="image">Image</th>
					<th class="sorting" onclick="sort('name')" id="name">Name</th>
					<th class="sorting" onclick="sort('cate_name')" id="cate_name">Cateloges</th>
					<th class="sorting" onclick="sort('favorite')" id="favorite">Favorite</th>
					<th class="sorting" onclick="sort('price')" id="price">Price</th>
					<th>Size</th>
			...
			<tbody>
				...
				<tr class="item_row<?=$i?> gradeX <?php ($k%2==0) ? print 'odd' : print 'even' ?>" role="row">
					...
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><img src="<?=resizeImage(PATH_URL.DIR_UPLOAD_PRODUCT.$v->image,150, 150)?>" /></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->name?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=$v->cate_name?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?php if($v->favorite) { echo 'Có';} else { echo 'Không';};?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=number_format($v->price)?></a></td>
					<td><a href="<?=PATH_URL_ADMIN.$module.'/update/'.$v->id?>"><?=json_encode(unserialize($v->price_size));?>
				...
				<?php $i++;}
					...
					<tr style="background:#c6c6c6;" class="item_row<?=$i?> gradeX" role="row">
						<td><input type="checkbox" id="item<?=$i?>" onclick="selectItem(<?=$i?>)" value="<?=$v->id?>"></td>
						<td class="center"><?=$k+1+$start?></td>
						<td><img src="<?=resizeImage(PATH_URL.DIR_UPLOAD_PRODUCT.$v->image,150, 150)?>" /></td>
						<td><?=$v->name?></td>
						<td><?=$v->cate_name?></td>
						<td><?php if($v->favorite) { echo 'Có';} else { echo 'Không';};?></td>
						<td><?=$v->price?></td>
